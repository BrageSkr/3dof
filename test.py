...
def ball_track(key1, queue):
    ...
    while True:
        ...
        cv2.circle(imgStack,
                   (center_point[0] + int(80 * np.cos(time.time()*2)), center_point[1] - int(80 * np.sin(time.time()*2))),
                   5, (20, 20, 255), 2)
...
def servo_control(key2, queue):
    ...
    while key2:
        ...
        reff_val_x = (80*np.cos(time.time()*2))/10
        reff_val_y = (80*np.sin(time.time()*2))/10
